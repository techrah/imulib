...
static inline double powdf(double m, double e)
{
    return pow(m, e);
}

static inline float powdf(float m, float e)
{
    return powf(m, e);
}
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, MemT> Matrix<rows, cols, MemT>::abs() const
{
...
template <int rows, int cols, class MemT>
Matrix<rows, cols, MemT> Matrix<rows, cols, MemT>::pow(typename MemT::elem_t e) const
{
    Matrix<rows, cols, Array<rows, cols, typename MemT::elem_t>> ret;
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++) ret(i, j) = powdf((*this)(i, j), e);
    return ret;
}

template <int rows, int cols, class MemT>
typename MemT::elem_t Matrix<rows, cols, MemT>::min() const
{
    typename MemT::elem_t res = (*this)(0,0);
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < cols; j++) res = MIN(res, (*this)(i, j));
    return res;
}
...
template <int rows, int cols, class MemT>
Diagonal<rows, MemT> Matrix<rows, cols, MemT>::Diag()
{
    BLA::Diagonal<rows, MemT> ref(storage);
    return ref;
}

template <int rows, int cols, class MemT>
ConstDiagonal<MemT> Matrix<rows, cols, MemT>::Diag() const
{
    ConstDiagonal<MemT> ref(storage);
    return ref;
}
